...
class AnemoiObsFuser(nn.Module):
    def __init__(
        ...
        self.fuser = instantiate(
            ...
            in_channels_src_obs = input_dim[self.obs_mesh_name] + self.graph.get_node_emb_size(self.obs_mesh_name),
            hidden_dim = self.num_channels,
    ...
    def forward(self, x: dict[str, Tensor], model_comm_group: Optional[ProcessGroup] = None) -> Tensor:
        ...
        assert (
            all(next(iter(x.values())).shape[0] == x[mesh].shape[0] for mesh in x.keys())
        ), "Batch shape must match for input datasets"
        batch_size = next(iter(x.values())).shape[0]
        assert (
            all(next(iter(x.values())).shape[2] == x[mesh].shape[2] for mesh in x.keys())
        ...
        for in_data_name, encoder in self.encoders.items():
            x_data_latent[in_data_name], x_latents[in_data_name] = self._run_mapper(
                encoder,
...
#            x_latents[in_data_name] = x_latent #.append(x_latent)
